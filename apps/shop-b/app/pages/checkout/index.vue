...
async function initializePayment() {
  ...
  try {
    ...
    elements.value = stripe.value.elements({
      ...
      appearance: {
        theme: 'stripe',
        variables: { colorPrimary: '#4F5E42', fontFamily: 'Manrope, system-ui, sans-serif' },
      },
    })
    ...
    <div class="checkout-layout">
      ...
      <div class="form-col">
        ...
        <section v-else class="section">
          ...
          <p class="digital-note">Your order contains only digital products — no shipping required.</p>
        </section>
      ...
      <aside class="summary-col">
        ...
        <div class="totals">
          ...
          <div class="total-row total-final">
            <span>Total</span>
            <span>{{ paymentReady ? `$${finalTotal.toFixed(2)}` : `$${(totalAmount + shippingCost).toFixed(2)}` }}</span>
          </div>
        </div>
      </aside>
    </div>
...
.page-title {
  ...
  font-size: 56px; font-weight: 400; letter-spacing: -0.6px;
  margin: 0 0 40px;
}
.checkout-layout {
...
.section-head { font-size: 11px; letter-spacing: 2px; text-transform: uppercase; color: var(--muted); margin: 0 0 16px; }
.input {
...
.summary-item { display: flex; gap: 12px; margin-bottom: 16px; padding-bottom: 16px; border-bottom: 1px solid var(--line); }
.summary-item:last-of-type { border-bottom: none; margin-bottom: 0; }
.summary-img { width: 56px; height: 56px; object-fit: cover; flex-shrink: 0; border-radius: 2px; }
.summary-img-ph { width: 56px; height: 56px; background: var(--line); flex-shrink: 0; border-radius: 2px; }
.summary-details { flex: 1; min-width: 0; }
.summary-title {
  font-family: var(--font-display); font-size: 15px; margin: 0 0 2px; line-height: 1.2;
  overflow: hidden; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;
}
.summary-attrs { font-size: 12px; color: var(--muted); margin: 0; }
.summary-price { font-size: 13px; margin: 4px 0 0; font-variant-numeric: tabular-nums; }
...
.total-final { color: var(--ink); font-size: 15px; font-weight: 600; padding-top: 10px; margin-top: 6px; border-top: 1px solid var(--line); }
</style>
